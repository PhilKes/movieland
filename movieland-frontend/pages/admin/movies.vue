    ...
    async deleteSelected(selected) {
      ...
      if (del === true) {
        ...
        selected.forEach((delMovie) => {
          promises.push(
            this.$repos.movies.remove(delMovie.movId).then((resp) => {
              return delMovie.idx;
            })
          );
        });
        Promise.all(promises).then((removedIdxs) => {
          removedIdxs.forEach((removeIdx) => {
            this.movies.splice(
              this.movies.findIndex((movie) => movie.idx === removeIdx),
              1
            );
          });
    ...
    async addMovie() {
      ...
      if (newMovies && newMovies !== false) {
        ...
        newMovies.forEach((newMovie) => {
          promises.push(
            this.$repos.movies.add(newMovie).then((addedMovie) => {
